...
Web翻訳は特別な設定なしですぐに使えるのだ！ ただし、1日に大量に利用すると利用制限がかかる場合があるのだ。また、この利用方式は非公式なものなので、認証方式が変更されたりサービス側の仕様が変わったりすると使えなくなることがあるのだ。その場合は次のアップデートを待つのだ。

<br>

**API翻訳ヒントなのだ！**

「API」と付いているモデルは、認証キーを発行しないと利用できないのだ！ 発行手続きが少し面倒な場合もあるのだが、その分安定性が高いのだ。サービスによっては無料プランが用意されていることもあるのだが、有料プランしか提供されていないモデルもあるのだ。
