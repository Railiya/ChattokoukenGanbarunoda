...
| Setting | Description |
|---|---|
| Start Translate on Buffered | 입력이 완료되고 버퍼 상태가 되면 자동으로 번역 요청을 보내는 것이다 |
| Auto Send Message on Translated | 번역이 완료되면 자동으로 입력 매크로를 시작하는 것이다 |
| Output Method | 입력 매크로의 작동 방식인 것이다 |
| Default Input Mode | 프로그램의 최초 한/영 상태인 것이다 |

- Output Method - Clipboard Paste : 클립보드 내용을 붙여넣는 방식인 것이다
- Output Method - Input Simulating : 클립보드 내용을 한 글자씩 입력하는 방식인 것이다 (붙여넣기가 되지 않는 게임에서 쓰는 것이다)
...
| Setting | Description |
|---|---|
| Model | 사용할 번역기의 모델인 것이다 |
| API Key | 번역기에 필요한 인증 키인 것이다 |
| Glossary Id | 용어집의 id인 것이다 |
| Source Language | 입력되는 언어인 것이다 |
| Destination Language | 번역될 언어인 것이다 |
| Translation Format | 번역된 문장이 클립보드에 복사되는 형식인 것이다 |
| Request Timeout | 번역을 요청하고 시간이 지나면 취소되는 것이다 |
...
| Setting | Description |
|---|---|
| Enabled | 오버레이를 켜거나 끄는 것이다 |
| Anchor | 오버레이가 표시되는 스크린의 기준점인 것이다 |
| Offset X,Y | 기준점으로 부터의 위치인 것이다 |
| Font Size | 폰트 크기인 것이다 |
| Opacity | 투명도인 것이다 |
...
| Setting | Description |
|---|---|
| Enabled | 소리 알림을 켜거나 끄는 것이다 |
| On Capturing Start | 입력이 시작될 때 알림을 주는 것이다 |
| On Translation Completed | 번역이 완료됐을 때 알림을 주는 것이다 |
| On Translation Failed | 문제가 생겨 번역이 실패했을 때 알림을 주는 것이다 |
| Volume | 알림의 음량인 것이다 |
...
| Setting | Description |
|---|---|
| Enable Capturing | 프로그램의 키 입력을 상태를 켜거나 끄는 것이다 |
| Capturing Toggle | 입력 시작과 끝인 것이다 (게임에서 쓴다면 채팅 단축키에 맞추는 것이다) |
| Translate | 번역 요청을 보내는 것이다 |
| Send Clipboard | 클립보드 내용 입력 매크로를 실행하는 것이다 |
...
| Setting | Description |
|---|---|
| Debug Echo Mode | 번역 요청을 보내지 않고 원문을 클립보드에 복사하는 것이다 |
| Write Log File | 상태가 변경될 때 마다 로그를 쓰는 것이다 |
