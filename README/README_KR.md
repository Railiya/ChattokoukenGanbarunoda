...
> 이 프로그램은 두 가지의 입력 방식이 있는 것이다. 그 중에서 직접 입력 방식은 지원되는 언어가 적은 것이다. 조금 불편할 수 있지만 직접 입력을 지원하지 않는 언어는 오버레이 입력을 사용해야 하는 것이다. 오버레이 입력은 윈도우 IME를 사용하기 때문에 언어 제약이 없는 것이다.

- 직접 입력 지원 언어 : 영어, 한국어
- 오버레이 입력 : 제약 없음 (윈도우 IME 사용)
...
이 프로그램은 게임이나 메신저의 채팅을 읽거나 가로채는 프로그램이 아닌 것이다. 그건 문제가 아주 많은 것이다. 그것과는 별개로 키 입력을 받아서 문장을 조립하는 것이다. 또는 오버레이에 있는 인풋 필드에 입력받는 것이다.
...
1. **[DISABLED]** : 프로그램의 키 입력이 꺼져있는 상태인 것이다. "캡처 활성화" 키로 켜거나 다시 끌 수 있는 것이다!
2. **[IDLE]** : 대기중인 상태인 것이다.
3. **[CAPTURING]** : 입력을 받고있는 중인 것이다. "채팅 토글" 키로 전환할 수 있는 것이다!
4. **[BUFFERED]** : 입력이 완료되고 문장이 저장된 상태인 것이다.
5. **\>\> TRANSLATING** : 번역 요청을 보내고 기다리는 중인 것이다! "번역 요청" 키로 요청을 보내는 것이다!
6. **[READY]** : 번역이 끝나서 클립보드에 복사된 것이다! "클립보드 전송" 키로 입력 매크로를 실행할 수 있는 것이다!
7. **[FAILED]** : 번역 요청이 실패한 것이다.. 이유는 여럿 있으니 로그 파일을 봐야하는 것이다.

> 오버레이를 활성화하면 현재 상태를 볼 수 있는 것이다!
...
> 4번과 6번 과정은 옵션을 설정하면 자동으로 진행시킬 수 있는 것이다! 두 옵션을 모두 켜면 다른 키 누를 필요 없이 채팅만 하면 되는 것이다! 이 과정은 어느정도 익숙해질 필요가 있으니 "고급 - 디버그 에코 모드" 를 켜고 메모장에서 연습해보는 걸 추천하는 것이다!
...
직접 입력은 게임이나 메신저의 채팅을 그대로 사용하는 것이다! 채팅은 채팅대로 입력하면서 프로그램이 키 입력을 주워담아 조합하고 그 내용을 번역하는 것이다. 그래서 실제 입력한 내용과 프로그램이 조합한 내용이 달라질 수 있는 것이다. 대표적인 예시가 한/영 상태와 마우스를 통한 커서 이동인 것이다.

<br>

**오버레이 입력 힌트인 것이다:**

오버레이 입력은 게임 또는 메신저와 프로그램간의 포커스 이동이 필요한 것이다! 프로그램의 인풋 필드에서 채팅을 입력하기 때문에 메신저는 괜찮지만 게임에 따라서는 불편함이 있을 수 있는 것이다. 채팅을 입력할 때는 프로그램으로 포커스가 이동되고 입력이 완료되면 다시 게임이나 메신저로 돌아가 먼저 원문을 전송하고 뒤이어 번역문을 전송하는 것이다. 포커스 이동간에는 약간의 딜레이가 있기 때문에 직접 입력에 비해서 살짝 느린 것이다.

게임에서 사용할 것이라면 다음을 확인해두는 것이다:

- 게임이 전체화면 (Fullscreen) 이면 안되는 것이다. 창 모드 (Windowed) 혹은 경계 없음 (Borderless) 여야 하는 것이다. 전체화면에서는 포커스 이동시 최소화되는 문제가 있는 것이다.
- 게임마다 포커스를 잃었을 때 음소거가 되거나 최소 프레임으로 설정되는 경우가 있는 것이다.
...
| 클립보드 전송 시 자동 캡처 전환 | 클립보드 전송이 완료되면 다음 텍스트 입력을 위해 자동으로 캡처 상태로 전환되는 것이다 |
| 대기 상태 생략 | 대기 상태를 생략하고 자동으로 캡처 상태로 전환되는 것이다 |
| 클립보드 전송 시 채팅 시작 입력 단계 생략 | 클립보드 전송 과정 중 채팅 시작 입력 단계를 생략하는 것이다 |
| (오버레이 입력) 원문 전송 시 채팅 시작 입력 단계 생략 | 오버레이 입력 사용 중 원문을 전송할 때 채팅 시작 입력 단계를 생략하는 것이다 |
| 입력 방식 | 채팅 원문을 입력하는 방식인 것이다 |
...
- 게임 : 버퍼 상태 진입 시 자동 번역 요청, 번역 완료 시 클립보드 자동 전송, 원문 전송 시 채팅 시작 입력 단계 생략

> 게임은 채팅을 입력할 때 활성화 -> 입력 -> 종료 순서이기 때문에 입력 단계를 생략하지 않고 사용하는 것이다. 번역을 대기하는 것이 싫다면 "번역 완료 시 클립보드 자동 전송" 만 꺼두는 것을 추천하는 것이다.

- 메신저 : 전부 다!

> 메신저는 번역을 대기한다고 불이익이 없는 것이다. 그러니 전부 자동화를 하는 것이다! 메신저에서는 항상 채팅이 활성화된 상태이기 때문에 입력이 끊이지 않도록 자동 캡처 상태 전환을 켜두는 것이다. 무언가 오작동이 되는 것 같다면 "클립보드 전송 시 채팅 시작 입력 단계 생략" 만 꺼두는 것을 추천하는 것이다.

<br>

### 오버레이 (Overlay)

> 프로그램의 현재 상태가 보이는 것이다. 상태에 따라 텍스트 색상도 바뀌는 것이다! 우측에는 프로그램의 입력 모드가 표시되는 것이다! 알파벳이면 'A' 한글이면 '가'로 표시되는 것이다! 오버레이 입력을 사용하면 언어 모드가 표시되지 않는 것이다.
...
웹 번역기는 별도의 설정 없이 바로 사용 가능한 것이다! 하지만 하루에 너무 많이 사용하면 제한을 받을 수 있는 것이다. 또한 이러한 사용 방식은 비공식적이기 때문에 이따금 인증 방식이 변경되거나 하면 사용할 수 없게 되는 것이다. 이 때는 다음 업데이트를 기다려야하는 것이다.

<br>

**API 번역기 힌트인 것이다**

API 가 붙은 모델은 인증 키를 발급해야 사용 가능한 것이다! 발급 절차가 까다로울 수 있지만 안정성이 높은 것이다. 모델에 따라 무료 플랜을 지원하기도 하지만 유료 플랜만 있는 모델도 있는 것이다.
...
> 프로파일과 별개로 프로그램의 환경을 구성하는 파일인 것이다. 프로그램을 처음 실행하면 "config.json" 파일이 만들어지는 것이다.

컨픽 파일은 직접 열어서 수정할 수 있는 것이다! 다른 인터페이스 언어를 사용하고 싶다면 파일을 수정하는 것이다! 언어를 지원하지 않으면 기본적으로 영어가 사용되는 것이다.

<br>

**프로파일**

> 프로파일은 프로그램을 실행했을 때 보이는 설정들을 저장해두는 파일인 것이다. 설정을 변경하면 자동으로 저장되는 것이다.

프로파일은 여러 개 만들어 사용할 수 있는 것이다. 지금은 임시적인 기능이라 직접 파일을 만들어야하는 것이다. 실행 파일이 있는 곳에서 "Profiles" 폴더에 들어가면 있는 "profile1.ckgprofile" 파일을 복사하고 이름을 "profile2", "profile3", "profile4" 처럼 지은 뒤 파일을 열어 "ProfileName" 을 수정하면 되는 것이다. 프로그램 상단 메뉴에서 불러올 수 있는 것이다.
...
지금 있는 임시 프로파일 기능과 달리 좌측에 사이드 메뉴로 프로파일 리스트를 추가하는 것이다.
...
직접 입력은 컴포저가 구현되지 않으면 사용할 수 없는 문제가 있다. 그럼에도 전세계 언어를 지원해야하기 때문에 근본적으로 다른 방법을 구상한 것이 오버레이 입력이다. 채팅 입력을 시작하면 게임이나 메신저에서 이 프로그램으로 포커스를 옮기고 오버레이 인풋 필드를 활성화하고 입력한 뒤 입력을 종료하면 다시 원래의 프로그램으로 포커스를 옮긴 뒤 저장해둔 원문을 전송하고 그 뒤는 직접 입력과 똑같이 번역문을 전송하는 방식이다. 인풋 필드는 윈도우 IME를 사용하기 때문에 입력 가능한 언어의 제약 없이 사용할 수 있는게 장점이다. 그럼에도 포커스를 이동하는 만큼 불편한 것이 좀 있다. 전체화면을 못 쓴다던지 안정성 같은 것 말이다. 
